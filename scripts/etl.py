...
class ExchangeETL():
    ...
    def __init__(self, config_path: str = "./config.yml") -> None:
        ...
        self.logger = set_logger("ExchangeETL",self.config['log']['log_etl_path'])
    ...
    def load_dataframe(self, index: int, data_path: str) -> bool:      
        try:
            ...
            self.logger.warning("The dataset in path %s was not loaded.", data_path)
            return False
    ...
    def change_column_type(self, index: int, column: str, type: str) -> None:
        try:
            if type == 'datetime':
                self.dataframes[index][column] = pd.to_datetime(self.dataframes[index][column])
                return True
        except Exception as exception:
            self.logger.error("%s", exception)
            self.logger.warning("The change in the column type was not performed", type)
            return False
    ...
    def processing(self) -> None:
        ...
        self.logger.info("The columns 'Date' and 'BRL' from dataframe0 was selected.")

        # converting the column 'Date' of dataframe0 to datetime
        self.change_column_type(0,'Date', 'datetime')
        self.logger.info("The type of the column 'Date' of dataframe0 was changed to datetime type.")

        # converting the column 'BRL' of dataframe0 to float
        self.dataframes[0]['BRL'] = self.dataframes[0]['BRL'].astype(float)
        self.logger.info("The type of the column 'BRL' of dataframe0 was changed to float type.")
        ...
        self.logger.info("The type of the column 'Date' of dataframe1 was changed to datetime type.")

        # sorting the values of column 'Date' of dataframe1
        self.dataframes[1].sort_values('Date', inplace= True, ascending=False)
        self.logger.info("The values of dataframe1 was successfully sorted by values in column 'Date'.")
        ...
        self.logger.info("The columns 'Date' and 'BRL' from dataframe1 was selected.")
        ...
        self.logger.info("The interval between 1994-2009 was selected in dataframe1.")

        # applying date filter in dataframe0
        filter3 = self.dataframes[0]['Date'].dt.year > 2008
        self.dataframes[0] = self.dataframes[0][filter3]
        self.logger.info("The dates above 2008 was selected in dataframe0.")

        # removing lines with ND value in dataframe1
        filter4 = self.dataframes[1]['BRL'] == 'ND'
        self.dataframes[1].drop(self.dataframes[1][filter4].index, inplace= True)
        self.logger.info("The rows with Non Data was removed from dataframe1 successfully.")

        # concatenating dataframe0 and dataframe1
        self.dataframes[2] = pd.concat([self.dataframes[0], self.dataframes[1]])
        self.logger.info("The dataframe0 and dataframe1 was concatenated in dataframe2.")
